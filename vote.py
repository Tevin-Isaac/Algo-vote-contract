...
class Voting(Application):
  ...
  num_of_voters: Final[ApplicationStateValue] = ApplicationStateValue(
    stack_type=TealType.uint64, default=Int(0)
  )
  can_vote: Final[AccountStateValue] = AccountStateValue(
    stack_type=TealType.uint64, default=Int(0)
  )
  vote_choice: Final[AccountStateValue] = AccountStateValue(
    stack_type=TealType.bytes, default=Bytes("abstain")
  )
  ...
  def optin_asset(
    self, 
    opt_txn: abi.AssetTransferTransaction
  ):
    return Seq(
      Assert(
  ...
  @external
  def create_registration_and_voting(
    self, 
